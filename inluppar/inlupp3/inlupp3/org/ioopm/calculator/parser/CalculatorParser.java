...
public class CalculatorParser {
    ...
    private final ArrayList<String> unallowedVars = new ArrayList<String>(Arrays.asList("Quit", "Vars", "Clear"));
     ...
     * @throws IOException          by nextToken() if it reads invalid input
     * @throws SyntaxErrorException if the token parsed cannot be turned into a
     *                              valid expression, the variable on rhs of '=' is
     *                              a number or invalid variable
     */
    ...
    /**
     * Checks wether the token read is an addition or subtraction and then continue
     * on with the right hand side of operator
     * 
     * @return a SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
    ...
    /**
     * Checks wether the token read is an Multiplication or Division and then
     * continue on with the right hand side of operator
     * 
     * @return a SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression term() throws IOException {
        ...
        this.st.nextToken();
        while (this.st.ttype == MULTIPLICATION || this.st.ttype == DIVISION) {
            int operation = st.ttype;
            this.st.nextToken();
    ...
    /**
     * Checks whether the token read is a parantheses and then continue on with the
     * expression inside of it or if the operation is an unary operation and then
     * continue on with the right hand side of that operator else if it's a
     * number/identifier
     * 
     * @return a SymbolicExpression to be evaluated
     * @throws IOException          by nextToken() if it reads invalid input
     * @throws SyntaxErrorException if the token parsed cannot be turned into a
     *                              valid expression, missing right parantheses
     */
    private SymbolicExpression primary() throws IOException {
        ...
        if (this.st.ttype == '(') {
            ...
            if (st.sval.equals(SIN) || st.sval.equals(COS) || st.sval.equals(EXP) || st.sval.equals(NEG)
                    || st.sval.equals(LOG)) {
    ...
    /**
     * Checks what type of Unary operation the token read is and then continues with
     * the expression that the operator holds
     * 
     * @return a SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
     ...
     * @throws IOException          by nextToken() if it reads invalid input
     * @throws SyntaxErrorException if the token parsed cannot be turned into a
     *                              valid expression, expected a number which is not
     *                              present
     */
